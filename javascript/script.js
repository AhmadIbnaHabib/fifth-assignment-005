...
callButtons.forEach(function(btn){
  btn.onclick = function(){
    if(star > 0){
      ...
      // Create new aside for history
      let aside = document.createElement('aside');
      aside.className = 'flex justify-between p-[24px] items-center w-[380px] h-[83px] bg-[#FAFAFA] mx-auto rounded-[8px] mb-[8px]';
      aside.innerHTML = `
        <div>
      ...
      // Append aside to history container
    //   historyContainer.appendChild(aside);


      // Append card to history container
            historyContainer.appendChild(aside);
...
clearButton.onclick = function(){
    historyContainer.innerHTML = ''; // শুধুমাত্র cards delete হবে
};
